 ...
 *
 * All EventSink calls are posted to the main looper — EventSink is not
 * thread-safe and must not be called from the capture thread.
 */
class FramePublisher(
    private val captureSink: () -> EventChannel.EventSink?,
    ...
    fun publish(frame: MutableAudioFrame) {
        ...
        // Copy into a fresh ByteArray for the main-thread post — the
        // serializationBuffer is reused on the next frame, so we can't
        // hand it to the closure. This is the only per-frame allocation;
        // it happens off the audio thread (mainHandler.post is deferred).
    ...
    fun emitWarning(code: String, message: String) {
        ...
        val payload = mapOf("type" to "warning", "code" to code, "message" to message)
        mainHandler.post { sink.success(payload) }
    }

    fun emitError(code: String, message: String) {
        val sink = eventSink() ?: return
        val payload = mapOf("type" to "error", "code" to code, "message" to message)
        mainHandler.post { sink.success(payload) }
    }

    fun emitStateChange(previous: String, current: String) {
        val sink = eventSink() ?: return
        val payload = mapOf("type" to "stateChanged", "previous" to previous, "current" to current)
        mainHandler.post { sink.success(payload) }
    }

    fun emitRouteChanged(previous: String, current: String) {
        val sink = eventSink() ?: return
        val payload = mapOf("type" to "routeChanged", "previous" to previous, "current" to current)
        mainHandler.post { sink.success(payload) }
    }
}
